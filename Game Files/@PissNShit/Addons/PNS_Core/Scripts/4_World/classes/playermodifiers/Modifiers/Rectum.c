...
class RectumMdfr: ModifierBase
{
	float m_LastRectumLevel;
	ref HumanMovementState m_MovementState = new HumanMovementState();
	
	override void Init()
	{
		//GetPNSLogger().WriteDebug("Init Rectum Modifier");
		Print("[PNS - LogHandler] :: [DEBUG] :: (Init Rectum Modifier");
		m_TrackActivatedTime = false;
		m_ID = eModifiersPNS.MDF_RECTUM;
	...
	override void OnTick(PlayerBase player, float deltaT)
	{
		...
		float Rectum = player.GetStatRectum().Get();
		float metabolic_speed = MiscGameplayFunctions.GetWaterMetabolicSpeed(m_MovementState.m_iMovement);
		
		float modifier = Rectum/PlayerConstants.SL_WATER_MAX + PlayerConstants.CONSUMPTION_MULTIPLIER_BASE;
		metabolic_speed *= modifier; //non linear shaping for consumption curve (comment out to have it linear)
		
		player.GetStatRectum().Add( (-metabolic_speed * deltaT) );
		
		if ( Rectum <= PlayerConstants.HIGH_RECTUM_THRESHOLD )
		{	
